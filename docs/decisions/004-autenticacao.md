...
## Atualização 2026-07-06 — implementação H1.1-H1.3 (backend-engineer, revisado por backend-architect)

Dois ajustes descobertos durante a implementação, ambos aprovados como extensão aditiva desta ADR (não mudam a decisão original, fecham lacunas de especificação):

**a) `sessao.substituida_por_id` (coluna nova, nullable, auto-referência).** A tolerância de 30 s do item acima foi especificada de forma genérica demais: sem marcar *por que* uma sessão foi revogada, o servidor também toleraria o reuso de um refresh revogado por logout, redefinição de senha ou remoção de membro — reabrindo uma sessão encerrada de propósito, caso o reuso caísse por acaso dentro da janela de 30 s. A coluna aponta para a sessão emitida na rotação legítima que revogou esta; a tolerância de 30 s só vale quando `substituida_por_id IS NOT NULL`. Efeito colateral bom: a coluna reifica a "família" de refresh tokens já citada no item 2 (revogar por roubo = revogar a família) como uma cadeia navegável, útil para investigação de incidente. Migration `0001_adiciona_substituida_por_id_sessao.sql` é aditiva (coluna nullable + FK, sem backfill) e reversível (`DROP COLUMN`/`DROP CONSTRAINT` triviais, sem perda de dado pré-existente).

**b) Claim `sid` (session id) no access token JWT.** O claim set do item 1 (`sub`, `organizacaoId`) não previa como `POST /auth/logout` — que não recebe corpo (`docs/api/identity.yaml`) — identificaria qual sessão revogar quando o membro tem múltiplas sessões ativas (múltiplos dispositivos). `sid` é claim adicional (padrão equivalente ao `jti`/`sid` de OIDC), não uma mudança de formato; claims de JWT não fazem parte do schema OpenAPI, então o contrato de `/auth/logout` não muda.

Limitação aceita para o MVP: se uma rotação de refresh acontecer em segundo plano (ex.: outra aba do mesmo navegador) entre a emissão do access token e o clique em "sair", o `sid` da aba que chama logout pode apontar para uma sessão já rotacionada (revogada por rotação legítima) — o logout revoga essa entrada, que já estava revogada, e a sessão ativa mais recente (rotacionada) permanece válida até expirar naturalmente. Não compromete o critério de aceite de H1.3 (remoção de membro continua revogando todas as sessões do membro, não depende do `sid`); revisitar apenas se surgir relato real de "logout não efetivo".
