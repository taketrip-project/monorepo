...
6. **[Adendo cto, 15/07/2026] Reaproveitamento de passageiro no fluxo público NUNCA sobrescreve dados existentes.** Achado do code-reviewer: `obterOuCriarPorOrganizacao` sobrescreve `nome`/`cpf` do passageiro existente quando o cadastro traz valores diferentes (`atualizarSeNecessario`) — correto no fluxo autenticado (ator confiável: o organizador), mas no fluxo público permitiria a qualquer anônimo com o link, sabendo o WhatsApp de um passageiro, renomear a pessoa e trocar o CPF dela via POST, corrompendo cadastro e lista de embarque (documento ANTT, peso legal). **Decisão: no fluxo público, reaproveitar o passageiro por WhatsApp sem sobrescrever nada — `cpf` só é gravado quando o campo existente está vazio; `nome` existente nunca muda; divergências são silenciosamente ignoradas (sem UI de conciliação no MVP — simplicidade vence).** O fluxo autenticado continua sobrescrevendo como hoje. Duplicar o passageiro (não reaproveitar) foi rejeitado: contradiz a regra do domínio (H1.9) e polui a base. Se dados reais mostrarem divergências frequentes, revisitar com sinalização ao organizador — pós-MVP.

## Adendo (cto, 16/07/2026) — ponto de embarque fora do formulário público nesta fase

Achado do code-reviewer na revisão do frontend: o POST público aceita `ponto_embarque_id` (opcional, validado contra a excursão), mas o `PontoEmbarquePublico` do GET público não expõe os `id`s dos pontos — o passageiro anônimo não tem como escolher, e toda reserva pública nasce sem ponto.

**Decisão: manter assim nesta fase.** A reserva pública já nasce pendente e a confirmação passa pelo organizador no WhatsApp — é nesse contato que o embarque é combinado; o PATCH autenticado de reserva já aceita `ponto_embarque_id` para o organizador registrar (a lista de embarque ANTT é montada sobre reservas confirmadas, então não fica comprometida). Reabrir backend+frontend já aprovados por um select a mais atrasa a entrega e adiciona fricção ao fluxo do passageiro sem dado real que justifique.

**Planejado para a próxima iteração da página pública:** expor `id` em `pontos_embarque` do GET público e adicionar a escolha no formulário — mudança pequena (mapper + um select). Priorizar se dados reais mostrarem que reservas sem ponto viram retrabalho de WhatsApp para o organizador. `ponto_embarque_id` permanece no contrato do POST exatamente para essa evolução ser puramente aditiva.
